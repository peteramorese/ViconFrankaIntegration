...
int main(int argc, char **argv) {
	...
	for (int i=0; i<Nobj; i++) {
                colObjVec[i].header.frame_id = "panda_link0";
        }
	...
	colObjVec[0].primitives[0].dimensions[0] = .09;
	colObjVec[0].primitives[0].dimensions[2] = .06;
	colObjVec[0].primitives[0].dimensions[1] = .12;
	...
	move_group.setEndEffectorLink("panda_link8");
        ...
        initpose.position.z = readz + .2;
	tf2::Quaternion initorient;
	initorient.setRPY(-5*M_PI/2, 3*M_PI/4, -M_PI/2);
        initpose.orientation = tf2::toMsg(initorient); 
        //initpose.orientation = p->pose.orientation; 
